...
namespace Services.Services
{
    public class BuildingService : BaseService, IBuildingService
    {
        ...
        {
            return await _dbContext.Buildings.AsNoTracking().OrderBy(x => x.Name)
                .ProjectTo<BuildingDto>(_mapper.ConfigurationProvider).ToListAsync();
        }

        public async Task<BuildingDto?> GetByIdAsync(int id)
        {
            return await _dbContext.Buildings.AsNoTracking().Where(x => x.Id == id)
                .ProjectTo<BuildingDto>(_mapper.ConfigurationProvider).FirstOrDefaultAsync();
        }
