...
namespace Services.Services
{
    public class EventTypeService : BaseService, IEventTypeService
    {
        ...
        {
            return await _dbContext.EventTypes.AsNoTracking().Select(x => new EventTypeDto
            {
        ...
        public async Task<EventTypeDto?> GetByIdAsync(int id)
        {
            return await _dbContext.EventTypes.AsNoTracking().Where(x => x.Id == id)
                .Select(x => new EventTypeDto { Id = x.Id, Name = x.Name, Description = x.Description }).FirstOrDefaultAsync();
        }

        public async Task<int> CreateAsync(CreateEventTypeDto dto)
        {
            var entity = new EventType { Name = dto.Name, Description = dto.Description ?? string.Empty };
        ...
        public async Task<bool> UpdateAsync(UpdateEventTypeDto dto)
        {
            ...
            entity.Name = dto.Name; entity.Description = dto.Description ?? string.Empty;
        ...
        public async Task<bool> DeleteAsync(int id)
        {
            ...
            if (entity == null) return false;
            _dbContext.EventTypes.Remove(entity); await _dbContext.SaveChangesAsync(); return true;
        }
    }
}