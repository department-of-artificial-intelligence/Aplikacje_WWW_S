    ...
    public class ReservationService : BaseService, IReservationService
    {
        ...
        public async Task<List<ReservationDto>> GetAllAsync()
        {
            return await _dbContext.Reservations
                ...
                {
                    Id = x.Id, RoomId = x.RoomId, RoomName = x.Room.Name,
        ...
        public async Task<int> CreateAsync(CreateReservationDto dto)
        {
            ...
            var entity = new Reservation
            {
                ...
                Status = ReservationStatus.Pending, // Domyślny status
        ...
        public async Task<bool> UpdateAsync(UpdateReservationDto dto)
        {
            ...
            // Sprawdzamy walidacje tylko jeśli czas, pokój lub event się zmieniły (lub dodajemy po prostu walidację z ID ignorując tę rezerwację)
        ...
        private async Task ValidateReservationAsync(int roomId, int eventId, DateTime startTime, DateTime endTime, int? reservationId = null)
        {
            ...
            if (room == null)
                throw new InvalidOperationException("Sala nie istnieje.");

            if (!room.IsActive)
                throw new InvalidOperationException("Sala nie jest aktywna.");

            var canAccommodate = await CanRoomAccommodateEventAsync(roomId, eventId);
            if (!canAccommodate)
                throw new InvalidOperationException("Pojemność sali jest niewystarczająca.");

            var hasConflict = await HasTimeConflictAsync(roomId, startTime, endTime, reservationId);
            if (hasConflict)
                throw new InvalidOperationException("Rezerwacje tej samej sali nie mogą nakładać się w czasie.");
        }
